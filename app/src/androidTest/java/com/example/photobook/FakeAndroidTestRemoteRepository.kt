...
class FakeAndroidTestRemoteRepository: IRemoteRepository
{
    ...
    ): Result?
    {
        ...
        {
            if (element.post_id == frameworkData.id && Login.currentUser?.uid == element.user_id)
            {
                ...
                val postVote = getPostVoteSnap(Login.currentUser!!.uid, frameworkData.id)
                if (postVote != null)
                {
                    if (postVote.isEmpty)
                    {
                        return Result(
                            ...
                            updatePost(
                                ...
                                listOf(
                                    mapOf(
                                        "id" to newPostVoteId,
                                        "user_id" to Login.currentUser!!.uid,
                    ...
                    else
                    {
                        ...
                        postVote.documents.mapNotNull { docSnapshot ->
                            ...
                            {
                                postVoteMap =
                                    if (doc.user_id == Login.currentUser!!.uid && doc.post_id == frameworkData.id)
                                    {
                                        mapOf(
                                            "id" to doc.id,
